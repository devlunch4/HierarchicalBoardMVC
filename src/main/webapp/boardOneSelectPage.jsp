							...
							<nav aria-label="member list Navigation" >
								<ul class="pagination justify-content-center m-0">
									<li class="page-item"><a class="page-link"
										href="${cp }/pagingBoard?page=1&pageSize=${pagevo.pageSize }&bcode=${bcode }"><i
											class="fas fa-angle-double-left"></i></a></li>


									<li class="page-item"><a class="page-link"
										href="${cp }/pagingBoard?page=
<c:choose>
<c:when test="${pagevo.getPage()- 1 <= 0 }">1</c:when>
<c:otherwise>${pagevo.getPage()- 1}</c:otherwise>
</c:choose>&pageSize=${pagevo.pageSize }&bcode=${bcode }"><i
									...
									<c:forEach begin="1" end="${pagination }" var="i">
										<c:choose>
											...
											<c:otherwise>
												<li class="page-item"><a class="page-link"
													href="${cp }/pagingBoard?page=${i }&pageSize=${pagevo.pageSize }&bcode=${bcode }">${i }</a></li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									<li class="page-item"><a class="page-link"
...
<c:otherwise>${pagevo.getPage()+ 1}</c:otherwise>
									...
									<li class="page-item"><a class="page-link"
										href="${cp }/pagingBoard?page=${pagination }&pageSize=${pagevo.pageSize }&bcode=${bcode }"><i
